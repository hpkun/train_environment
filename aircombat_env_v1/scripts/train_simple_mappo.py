...
def save_checkpoint(path,model,adapter,args,env_steps):
    contract={"scenario":args.scenario,"hetero_perception_mode":getattr(args,"hetero_perception_mode","paper_fused"),
      "hetero_reward_mode":getattr(args,"hetero_reward_mode","paper_table1_v2"),"reward_contract_version":"heterogeneous_reward_v2",
      "obs_dim":adapter.obs_dim,"state_dim":adapter.state_dim,"action_dim":adapter.action_dim,"agent_ids":adapter.agent_ids}
...
def main():
    ...
    role_returns={aid:0. for aid in adapter.agent_ids};component_sums={key:0. for key in EPISODE_COMPONENT_FIELDS[10:26]}
    try:
        while env_steps<args.total_env_steps:
            ...
            for _ in range(rollout):
                ...
                if team_done:
                    episodes_completed+=1;uav_returns=[role_returns.get(aid,0.) for aid in ("red_uav_0","red_uav_1")];mean_uav=float(np.mean(uav_returns)) if args.scenario=="simple_paper_3v2_hetero" else 0.
                    episode_row={"episode":episodes_completed,"env_steps":env_steps,"length":episode_length,"winner":info["winner"],"termination_reason":info["termination_reason"],"team_return":episode_return,
                      "mav_return":role_returns.get("red_mav_0",0.),"red_uav_0_return":role_returns.get("red_uav_0",0.),"red_uav_1_return":role_returns.get("red_uav_1",0.),"mean_red_uav_return":mean_uav,**component_sums,
                      "red_missile_kills":info.get("red_missile_kills",0),"blue_missile_kills":info.get("blue_missile_kills",0),"mav_alive":info.get("mav_alive",False),"red_uav_alive":info.get("red_uav_alive",0),
                      "numerical_invalid":info["numerical_invalid"],"crashes":info["red_crashes"]+info["blue_crashes"],"boundary_deaths":info["boundary_deaths"]}
