...
def evaluate_model(model,scenario,episodes,device,deterministic=True,seed=1,return_rows=False,hetero_perception_mode="paper_fused"):
    ...
    try:
        ...
        for episode in range(episodes):
            ...
            env=SimpleTAMCombatEnv(scenario,"red",hetero_perception_mode=hetero_perception_mode) if scenario=="simple_paper_3v2_hetero" else SimpleTAMCombatEnv(scenario,"red")
...
def load_checkpoint(path,scenario,device,hetero_perception_mode="paper_fused"):
    checkpoint=torch.load(path,map_location=device,weights_only=False)
    env=SimpleTAMCombatEnv(scenario,hetero_perception_mode=hetero_perception_mode) if scenario=="simple_paper_3v2_hetero" else SimpleTAMCombatEnv(scenario)
...
def main():
    p=argparse.ArgumentParser();p.add_argument("--model",required=True);p.add_argument("--scenario",choices=("simple_paper_1v1","simple_paper_2v2","simple_paper_3v2_hetero"),required=True);p.add_argument("--episodes",type=int,default=5);p.add_argument("--device",default="auto");p.add_argument("--deterministic",action="store_true");p.add_argument("--seed",type=int,default=1);p.add_argument("--hetero-perception-mode",choices=("paper_fused","uav_only_ablation"),default="paper_fused");a=p.parse_args()
    device=resolve_device(a.device);model=load_checkpoint(a.model,a.scenario,device,a.hetero_perception_mode);print(json.dumps(evaluate_model(model,a.scenario,a.episodes,device,a.deterministic,a.seed,False,a.hetero_perception_mode),indent=2))
