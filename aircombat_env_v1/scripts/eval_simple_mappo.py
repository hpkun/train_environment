...
def evaluate_model(model,scenario,episodes,device,deterministic=True,seed=1):
    rows=[]
    for episode in range(episodes):
        env=SimpleTAMCombatEnv(scenario,"red");adapter=SimpleMAPPOAdapter(env);obs,state,info=adapter.reset(seed=seed+episode)
        active=np.ones(adapter.num_agents,np.float32);episode_return=0.
        try:
            for step in range(1000):
                ...
                obs,state,rewards,done,next_active,info=adapter.step(actions.cpu().numpy())
            ...
            rows.append({"return":episode_return,"length":step+1,"winner":info["winner"],"timeout":info["termination_reason"]=="timeout",
              ...
              "numerical_invalid":info["numerical_invalid"],"envelope":info["flight_envelope_violation"]})
        finally:env.close()
    n=max(len(rows),1)
    return {"episodes":len(rows),"mean_return":float(np.mean([r["return"] for r in rows])),"mean_episode_length":float(np.mean([r["length"] for r in rows])),
