...
def _action_dim_from_env(env) -> int:
    dims = {int(env.action_space[rid].shape[0]) for rid in env.red_ids}
    if len(dims) != 1:
        raise ValueError(f"red agents must share one action dimension, got {sorted(dims)}")
    return dims.pop()
...
def main() -> None:
    ...
    if checkpoint_action_dim != action_dim:
        raise ValueError(
            f"checkpoint action_dim={checkpoint_action_dim} does not match environment action_dim={action_dim}"
        )
    policy = _build_policy_from_meta(meta, device, action_dim)
