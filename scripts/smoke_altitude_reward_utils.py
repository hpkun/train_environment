...
def main() -> None:
    ...
    _assert_close(altitude_reward_paper_eq17(10000.0), 0.1)
    _assert_close(altitude_reward_paper_eq17(12000.0), 0.1)

    for _dz, value in sample_altitude_table(altitude_reward_paper_eq17):
        assert 0.0 <= value <= 1.0

    expected_pairwise = (
        altitude_reward_paper_eq17(6000.0 - 4000.0)
        + altitude_reward_paper_eq17(6000.0 - 1000.0)
    ) / 2.0
    _assert_close(
    ...
    _assert_close(
        altitude_reward_pairwise_mean_eq17(6000.0, [4000.0, 1000.0]),
        expected_pairwise,
    )
    ...
    print("paper eq17 altitude table:")
    for dz, value in sample_altitude_table(altitude_reward_paper_eq17):
        print(f"  dz={dz:8.1f} m -> {value: .6f}")
