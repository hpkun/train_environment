...
# 【修改】cuda改CPU
# 【修改】 --n-rollout-threads 从 32 降到了 2，保护 Mac 内存不崩溃
# 【修改】删除了--use-wandb 及相关账号信息，直接在本地静默跑，不再弹窗卡进度

python train/train_jsbsim.py \
    --env-name ${env} --algorithm-name ${algo} --scenario-name ${scenario} --experiment-name ${exp} \
    --seed ${seed} --n-training-threads 1 --n-rollout-threads 2 --log-interval 1 --save-interval 1 \
