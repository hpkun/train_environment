...
class HeteroUavCombatEnv(UavCombatEnv):
    ...
    def _compute_rewards(self) -> tuple[dict, dict]:
        ...
        # ---- paper_role_reward_v1: tam_brma_role_reward_refined_v1 ----
        if self.hetero_reward_mode == "paper_role_reward_v1":
            # ---- Terminal: zero r_end always (timeout + elimination) ----
            for aid in self.agent_ids:
                old_end = components[aid].get("r_end", 0.0)
                if old_end != 0.0:
                    components[aid]["r_end_raw_removed"] = float(old_end)
                    base_rewards[aid] = base_rewards.get(aid, 0.0) - old_end
                    components[aid]["r_end"] = 0.0
                components[aid].setdefault("terminal_timeout_reward", 0.0)
                components[aid].setdefault("terminal_elimination_reward", 0.0)

            # ---- MAV: remove base r_adv ----
            ...
            # ---- UAV: weaken base r_adv ----
            for rid in self.red_ids:
                if self.agent_roles.get(rid, "") == "mav":
                    continue
                old_adv = components[rid].get("r_adv", 0.0)
                if old_adv != 0.0:
                    raw_adv = old_adv / 0.15
                    r_adv_uav = 0.02 * float(np.clip(raw_adv, -2.0, 2.0))
                    components[rid]["r_adv_removed"] = float(old_adv)
                    components[rid]["r_adv_uav"] = r_adv_uav
                    base_rewards[rid] = base_rewards.get(rid, 0.0) - old_adv + r_adv_uav
                    components[rid]["r_adv"] = 0.0

            # ---- Replace old boundary and altitude ----
            for rid in self.red_ids:
                sim = self.red_planes.get(rid)
