...
| `brma_recurrent_masked` | BRMA-style entity attention encoder | GRUCell | Random scale mask and/or biased mask | MAV head + shared UAV head | 480-dim MLP |
...
-> Mask module
   ...
   -> optional random scale mask during training
   -> optional biased mask generator when explicitly enabled
...
| Random scale mask | Randomly drops valid non-self entities only in training mode. | Disabled in eval mode; used as an opt-in robustness mechanism. |
...
| Random mask | Random friendly/enemy masking for scale robustness. | Random non-self entity dropout during training, disabled during eval. | Partially aligned | Current implementation uses current fixed entity slots and simplified sampling. |
