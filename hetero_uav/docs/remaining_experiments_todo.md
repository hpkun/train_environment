...
|---|---|---|---|---|
| A | Complete `brma_recurrent_masked` random-mask training to 500k or 1M | Must | Final main-method candidate | Use non-time-limited terminal; preserve intermediate eval/checkpoints. |
| B | Optional biased-mask 500k probe | Optional | Ablation for biased mask generator | Run only after random-mask path is stable enough. |
...
| K | Full BRMA biased mask objective | Can drop | Algorithmic completeness | Current biased mask is forward/logging only; future work. |

## Recommended Immediate Sequence

1. Finish one clean random-mask run with intermediate checkpoint/eval logging.
...
4. Only then decide whether biased-mask 500k is worth running.
