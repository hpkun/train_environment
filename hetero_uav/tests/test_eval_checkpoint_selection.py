...
def test_eval_checkpoint_scores_select_different_configs():
    from eval_checkpoint_selection import compute_eval_scores, best_metric_name, eval_record_label
    ...
    records = [
        ...
        {
            "config": "uav_env/JSBSim/configs/hetero_mav_shared_geo_7v6_f22_pid.yaml",
            "red_win_rate": 0.2,
    ...
    assert scores["score_7v6"] == pytest.approx(0.5)
    assert scores["score_combined"] == pytest.approx(0.34)
    assert eval_record_label(records[2]) == "7v6"
    assert best_metric_name("best_5v4") == "score_5v4"
    assert best_metric_name("best_7v6") == "score_7v6"
...
def test_eval_checkpoint_meta_contains_step_and_metrics(tmp_path):
    ...
    records = [
        ...
        {"config": "7v6.yaml", "red_win_rate": 0.3, "red_missile_hits_mean": 1.0, "blue_dead_mean": 0.5},
    ]
