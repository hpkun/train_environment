...
def test_smoke():
    r=subprocess.run([PY,"scripts/smoke_main_mappo_baseline_1m_fast.py"],cwd=ROOT,env=_env(),text=True,capture_output=True,encoding="utf-8",errors="replace",timeout=600)
    assert r.returncode==0,f"{r.stdout[-500:]} {r.stderr[-500:]}"
    out=ROOT/"outputs/test_main_mappo_baseline_1m_fast"
