...
def _summarize(
    ...
    return {
        ...
        "shooter_alive_rate": float(np.mean([bool(r.get("shooter_alive")) for r in rows])),
        "shooter_has_missile_rate": float(np.mean([bool(r.get("shooter_has_missile", r.get("has_missile"))) for r in rows])),
        "geometry_ok_rate": float(np.mean([
            bool(r.get("range_ok")) and bool(r.get("ao_ok")) and bool(r.get("ta_ok"))
            for r in rows
        ])),
...
def _write_md(path: Path, summary: dict[str, Any]) -> None:
    lines = [
        ...
        f"- shooter_alive_rate: `{summary.get('shooter_alive_rate')}`",
        f"- shooter_has_missile_rate: `{summary.get('shooter_has_missile_rate')}`",
        f"- geometry_ok_rate: `{summary.get('geometry_ok_rate')}`",
...
def main() -> int:
    ...
    parser.add_argument("--opponent-policy", default="brma_rule",
                        choices=["zero", "random", "rule_nearest", "greedy_fsm", "brma_rule", "brma_rule_safe_pursuit", "tam_greedy_easy", "brma_rule_safe_pursuit_easy", "fixed_route", "tam_greedy_rule"])
