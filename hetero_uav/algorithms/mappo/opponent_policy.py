...
class OpponentPolicy:
    ...
    Supported modes:
    - ``zero``: all blue actions are zero (smoke/debug only).
    - ``random``: sample uniformly from [-1, 1] (debug only).
    - ``rule_nearest``: steer toward nearest red entity (debug only).
    ...
    def act(self, obs_dict: dict, blue_ids: list[str],
        ...
        if self.mode == "brma_rule":
            return self._brma_rule_actions(obs_dict, blue_ids, env, pursuit_mode="delta10")
        return {
